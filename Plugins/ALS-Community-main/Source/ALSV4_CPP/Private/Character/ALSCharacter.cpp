...
void AALSCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(AALSCharacter, NPCStruct);
}
...
bool AALSCharacter::CanBeSeenFrom(const FVector& ObserverLocation, FVector& OutSeenLocation,
	...
	int32* UserData) const
{
	...
	FVector SightTargetLocation = GetMesh()->GetSocketLocation("spine_03");
	...
	if (!hit || (IsValid(HitResults.GetActor())) && HitResults.GetActor()->IsOwnedBy(this))
	{
