...
/*
void FALSAnimInstanceProxy::InitializeObjects(UAnimInstance* InAnimInstance)
{
	FAnimInstanceProxy::InitializeObjects(InAnimInstance);

	//ProxyCharacter = Cast<AALSBaseCharacter>(InAnimInstance->TryGetPawnOwner());
	//if(ProxyCharacter == nullptr) { return; }
}


void FALSAnimInstanceProxy::PreUpdate(UAnimInstance* InAnimInstance, float DeltaSeconds)
{
	FAnimInstanceProxy::PreUpdate(InAnimInstance, DeltaSeconds);
	//ProxyCharacter = Cast<AALSBaseCharacter>(InAnimInstance->TryGetPawnOwner());
	
	UALSCharacterAnimInstance* AnimInstance = Cast<UALSCharacterAnimInstance>(InAnimInstance);
	ProxyCharacter = AnimInstance->Character;
	ProxyCharacterInformation = AnimInstance->CharacterInformation;
	ProxyLayerBlendingValues = AnimInstance->LayerBlendingValues;
	ProxyMovementState = AnimInstance->MovementState;
	ProxyMovementAction = AnimInstance->MovementAction;
	ProxyStance = AnimInstance->Stance;
	ProxyRotationMode = AnimInstance->RotationMode;
	ProxyGait = AnimInstance->Gait;
	ProxyOverlayState = AnimInstance->OverlayState;
	ProxyGroundedEntryState = AnimInstance->GroundedEntryState;
	ProxyAimingValues = AnimInstance->AimingValues;
	ProxyGrounded = AnimInstance->Grounded;
	ProxyTurnInPlaceValues = AnimInstance->TurnInPlaceValues;
	TS_UpdateRate = GetSkelMeshComponent()->AnimUpdateRateParams->UpdateRate;
	TS_OwningComponent = AnimInstance->GetOwningComponent();

	if(ProxyCharacter)
	{
		ProxyCharacterInformation.MovementInputAmount = ProxyCharacter->GetMovementInputAmount();
		ProxyCharacterInformation.bHasMovementInput = ProxyCharacter->HasMovementInput();
		ProxyCharacterInformation.bIsMoving = ProxyCharacter->IsMoving();
		ProxyCharacterInformation.Acceleration = ProxyCharacter->GetAcceleration();
		ProxyCharacterInformation.AimYawRate = ProxyCharacter->GetAimYawRate();
		ProxyCharacterInformation.Speed = ProxyCharacter->GetSpeed();
		ProxyCharacterInformation.Velocity = ProxyCharacter->GetCharacterMovement()->Velocity;
		ProxyCharacterInformation.MovementInput = ProxyCharacter->GetMovementInput();
		ProxyCharacterInformation.AimingRotation = ProxyCharacter->GetAimingRotation();
		ProxyCharacterInformation.CharacterActorRotation = ProxyCharacter->GetActorRotation();
		ProxyCharacterInformation.ViewMode = ProxyCharacter->GetViewMode();
		ProxyCharacterInformation.PrevMovementState = ProxyCharacter->GetPrevMovementState();
	
		ProxyLayerBlendingValues.OverlayOverrideState = ProxyCharacter->GetOverlayOverrideState();

		ProxyMovementState = ProxyCharacter->GetMovementState();
		ProxyMovementAction = ProxyCharacter->GetMovementAction();
		ProxyStance = ProxyCharacter->GetStance();
		ProxyRotationMode = ProxyCharacter->GetRotationMode();
		ProxyGait = ProxyCharacter->GetGait();
		ProxyOverlayState = ProxyCharacter->GetOverlayState();
		ProxyGroundedEntryState = ProxyCharacter->GetGroundedEntryState();
	} else
	{
		UE_LOG(LogTemp, Error, TEXT("ProxyCharacter is NOT VALID in PreUpdate()"));
	}
}


void FALSAnimInstanceProxy::Update(float DeltaSeconds)
{
...
void FALSAnimInstanceProxy::PostUpdate(UAnimInstance* InAnimInstance) const
{
	FAnimInstanceProxy::PostUpdate(InAnimInstance);
	UALSCharacterAnimInstance* AnimInstance = Cast<UALSCharacterAnimInstance>(InAnimInstance);
	AnimInstance->Character = ProxyCharacter;
	AnimInstance->CharacterInformation = ProxyCharacterInformation;

	AnimInstance->LayerBlendingValues = ProxyLayerBlendingValues;
	AnimInstance->MovementState = ProxyMovementState;
	AnimInstance->MovementAction = ProxyMovementAction;
	AnimInstance->Stance = ProxyStance;
	AnimInstance->RotationMode = ProxyRotationMode;
	AnimInstance->Gait = ProxyGait;
	AnimInstance->OverlayState = ProxyOverlayState;
	AnimInstance->GroundedEntryState = ProxyGroundedEntryState;

	AnimInstance->AimingValues = ProxyAimingValues;
	AnimInstance->Grounded = ProxyGrounded;
	AnimInstance->TurnInPlaceValues = ProxyTurnInPlaceValues;
}
*/
...
void UALSCharacterAnimInstance::NativeInitializeAnimation()
{
	...
	IkFootL_BoneName = (!IsCustomSkeleton) ? NAME_ik_foot_l : NAME_VB_ik_foot_l;
	IkFootR_BoneName = (!IsCustomSkeleton) ? NAME_ik_foot_r : NAME_VB_ik_foot_r;
}
...
void UALSCharacterAnimInstance::NativePostEvaluateAnimation()
{
...
bool UALSCharacterAnimInstance::CanRotateInPlace() const
{
	return (RotationMode.Aiming() ||
		CharacterInformation.ViewMode == EALSViewMode::FirstPerson) && !IsRider;
}
...
bool UALSCharacterAnimInstance::CanTurnInPlace() const
{
	return RotationMode.LookingDirection() &&
		...
		GetCurveValue(NAME_Enable_Transition) >= 0.99f && !IsRider;
}
...
void UALSCharacterAnimInstance::UpdateAimingValues(float DeltaSeconds)
{
	...
	Delta.Normalize();
	AimingValues.AimingAngle.X = Delta.Yaw /*+ AdditionalAimOffset.Yaw*/;
	AimingValues.AimingAngle.Y = Delta.Pitch /*+ AdditionalAimOffset.Pitch*/;
...
void UALSCharacterAnimInstance::PlayQueuedTurnInPlaceAnimation()
{
	check(IsInGameThread())

	if(!IsValid(QueuedTurnInPlace.QueuedTurnInPlaceAsset.Animation))
	{
		return;
	}

	// Step 3: If the Target Turn Animation is not playing or set to be overriden, play the turn animation as a dynamic montage.
	if (!QueuedTurnInPlace.OverrideCurrent && IsPlayingSlotAnimation(QueuedTurnInPlace.QueuedTurnInPlaceAsset.Animation, QueuedTurnInPlace.QueuedTurnInPlaceAsset.SlotName))
	{
		return;
	}
	PlaySlotAnimationAsDynamicMontage(QueuedTurnInPlace.QueuedTurnInPlaceAsset.Animation, QueuedTurnInPlace.QueuedTurnInPlaceAsset.SlotName, 0.2f, 0.2f,
									  QueuedTurnInPlace.QueuedTurnInPlaceAsset.PlayRate * QueuedTurnInPlace.PlayRateScale, 1, 0.0f, QueuedTurnInPlace.StartTime);

	// Step 4: Scale the rotation amount (gets scaled in AnimGraph) to compensate for turn angle (If Allowed) and play rate.
	if (QueuedTurnInPlace.QueuedTurnInPlaceAsset.ScaleTurnAngle)
	{
		Grounded.RotationScale = (QueuedTurnInPlace.QueuedTurnAngle / QueuedTurnInPlace.QueuedTurnInPlaceAsset.AnimatedAngle) * QueuedTurnInPlace.QueuedTurnInPlaceAsset.PlayRate * QueuedTurnInPlace.PlayRateScale;
	}
	else
	{
		Grounded.RotationScale = QueuedTurnInPlace.QueuedTurnInPlaceAsset.PlayRate * QueuedTurnInPlace.PlayRateScale;
	}

	//Reset the queued turn in place
	QueuedTurnInPlace.QueuedTurnInPlaceAsset.Animation = nullptr;
	QueuedTurnInPlace.QueuedTurnInPlaceAsset.SlotName = NAME_None;
	QueuedTurnInPlace.QueuedTurnAngle = 0.0f;
}
